...
def load_and_evaluate(model_name, X_test, y_test, selector=None):
    ...
    if selector:
        ...
        lr_reduced_pred = (lr_reduced_prob >= 0.5).astype(
            int
        )  # Turn regression into 2 classes
...
def find_best_params(X_train, y_train, X_train_reduced, selector):
    ...
    grid_search = GridSearchCV(
        estimator=rf, param_grid=params, cv=5, scoring="accuracy", verbose=2, n_jobs=-1
    )
