...
class MissingDataVisualizer(BaseVisualizer):
    ...
    def visualize(self, missing_percentage_sorted: pd.Series, output_path: str) -> None:
        ...
        ax = sns.barplot(
            x=missing_percentage_sorted.index, y=missing_percentage_sorted.values
        )
        ...
        plt.xlabel(
            config.X_LABEL, fontsize=config.X_LABEL_FONT_SIZE, labelpad=config.LABEL_PAD
        )
        plt.ylabel(
            config.Y_LABEL, fontsize=config.Y_LABEL_FONT_SIZE, labelpad=config.LABEL_PAD
        )
